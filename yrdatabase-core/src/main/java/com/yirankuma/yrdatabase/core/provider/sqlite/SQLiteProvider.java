package com.yirankuma.yrdatabase.core.provider.sqlite;
 ...
 */
@Slf4j
public class SQLiteProvider implements PersistProvider {
    ...
    public SQLiteProvider(DatabaseConfig.PersistConfig.SQLiteConfig config) {
        this.dbPath = config.getFile();
    }
    ...
    public CompletableFuture<Void> initialize() {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                log.info("SQLite connected successfully to {}", dbPath);
            } catch (Exception e) {
                log.error("Failed to connect to SQLite: {}", e.getMessage());
                connected = false;
    ...
    private <T> CompletableFuture<T> executeAsync(SqlFunction<T> function) {
        if (!connected || connection == null) {
            return CompletableFuture.failedFuture(new IllegalStateException("SQLite not connected"));
        }
        ...
        return CompletableFuture.supplyAsync(() -> {
            synchronized (lock) {
                try {
                    ...
                    log.error("SQLite operation failed: {}", e.getMessage());
    ...
    public CompletableFuture<Boolean> createTable(String tableName, Map<String, String> schema) {
        ...
        return executeAsync(conn -> {
            ...
            for (Map.Entry<String, String> entry : schema.entrySet()) {
                ...
                String sqliteType = convertToSQLiteType(entry.getValue());
                String columnDef = "`" + entry.getKey() + "` " + sqliteType;
    ...
    private String convertToSQLiteType(String mysqlType) {
        ...
        if (upper.contains("VARCHAR") || upper.contains("TEXT") || upper.contains("CHAR")) {
            return upper.contains("PRIMARY KEY") ? "TEXT PRIMARY KEY" : "TEXT";
        } else if (upper.contains("INT") || upper.contains("BIGINT") || upper.contains("TINYINT")) {
            return upper.contains("PRIMARY KEY") ? "INTEGER PRIMARY KEY" : "INTEGER";
    ...
    public void close() {
        ...
        try {
            ...
            log.error("Error closing SQLite connection: {}", e.getMessage());
        }
    }
}
